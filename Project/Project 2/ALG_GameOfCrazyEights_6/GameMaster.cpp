 ...
 * Last Edited: June 8th, 2024, 8:48 PM
 */
...
void GameMaster::startGame() {
    ...
    Graph cardPoints;                               //Graph of cardPoints
    ...
    *round = 1;
    ...
    while(!gameOver) {
        ...
        for(int i = 0; i < playerSize; i++) {
            //Edge newEdge(NULL, NULL, topCard->getPtVal());
            //cardPoints.insertEdge(newEdge);
        ...
        for(int j = 0; j < players[i+1].getCardAmount(); j++) {
            for(it = values.begin(); it != values.end(); it++) {
                ...
                if(card->getValue() == *it) {
                    ...
                    total += card->getPtVal();
                    tree.root = tree.insert(tree.root, card->getPtVal());
                    break;
                } //End of independent if
            } //End of nested for loop
        } //End of nested for loop
