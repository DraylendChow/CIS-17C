...
GameMaster::GameMaster(int playerSize) {
    //players = new Player*[playerSize];  //Dynamically create array of Players //DELETE
    ...
    for(int i = 0; i < playerSize; i++) {
        ...
        players[i+1].setNum(i+1);
        players[i+1].setCardAmount(5);
        players[i+1].getRecord()->setUsername(name);
    } //End of for loop
...
void GameMaster::dealCards() {
    ...
    for(int i = 0; i < playerSize; i++) {
        Card **c = new Card*[players[i+1].getCardAmount()];
        players[i+1].setHand(c);
    } //End of for loop
    ...
    for(int i = 0; i < (playerSize*5);) {
        for(int j = 0; j < playerSize; j++) {
            players[j+1].getHand()[currentCard] = theCards[i];      //Assign card to players hand
            i++;
                            ...
                            if(players[i+1].getCardAmount() == 0) {
                                ...
                                for(int j = 0; j < playerSize; j++) {
                                    if(i != j) {
                                        ...
                                        //For winner
                                        winnerPos = j;
                                        players[j+1].getRecord()->setTotalPts(calcPts(playerSize)); //Tally points
                    ...
                    if(cardCount > 0) {
                        ...
                        if(check == false) {
                            ...
                            while(!keepDraw) {
                                ...
                                players[i+1].drawCard(theCards[currentCount]); //Draw card
                                counter++;
                            ...
                            cout << "You have drawn a card: " << *(theCards)[currentCount] << endl;
                            players[i+1].drawCard(theCards[currentCount]);
                            cardCount--;    //Amount of cards left
                            currentCount++; //Current index in shuffled cards
                            playerDone = false; //Change to repeat user turn
                        } //End of dependent else
...
int GameMaster::calcPts(int) {
    ...
    for (auto i : playerPts) {
        cout << "Player " << count << " had " << i << " points." << endl;
        count--;
    }
